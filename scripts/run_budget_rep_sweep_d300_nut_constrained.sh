#!/usr/bin/env bash
# scripts/run_budget_rep_sweep_d300_nut_constrained.sh
#
# Run the MimicGen budget × replicate sweep for the 300-demo baseline with
# nut-constrained initial poses.
#
# Identical experiment design to run_budget_rep_sweep_d300.sh (d300 unconstrained)
...
#
# Uses the same upstream run (seed=1 d300) via a run_clustering symlink in the
...
LOG="${WORKTREE}/logs/budget_rep_sweep_d300_nut_constrained.log"

cd "${WORKTREE}"

echo "[$(date '+%H:%M PDT')] === Budget rep sweep d300 nut-constrained started ===" | tee -a "${LOG}"
...
conda run -n policy_doctor --no-capture-output \
    python -m policy_doctor.scripts.run_pipeline \
        ...
        "experiment=mimicgen_square_rep_sweep_apr26_d300_nut_constrained" \
...
echo "[$(date '+%H:%M PDT')] === Budget rep sweep d300 nut-constrained complete ===" | tee -a "${LOG}"
