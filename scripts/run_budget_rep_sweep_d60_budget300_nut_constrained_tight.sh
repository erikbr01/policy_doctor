...
# TIGHT nut-constrained initial poses and generation budget=300.
#
# Tight constraint: nut x/y ±40 mm, z_rot ±30° (identical to the loose
# constrained run but with narrower bounds).
#
...
LOG="${WORKTREE}/logs/budget_rep_sweep_d60_budget300_nut_constrained_tight.log"
mkdir -p "${WORKTREE}/logs"

cd "${WORKTREE}"

echo "[$(date '+%H:%M %Z')] === Budget rep sweep d60 budget300 nut-constrained-tight started ===" | tee -a "${LOG}"
...
echo "[$(date '+%H:%M %Z')] === Budget rep sweep d60 budget300 nut-constrained-tight complete ===" | tee -a "${LOG}"
