...
def _parameter_arnoldi(
    ...
    iteration_callback: Optional[
        Callable[[List[Tuple[Tensor, ...]], Tensor, int], None]
    ] = None,
    initial_qs: Optional[List[Tuple[Tensor, ...]]] = None,
    initial_H_filled: Optional[Tensor] = None,
    ...
    Args:
        ...
        iteration_callback (callable, optional): If provided, called after each
                completed iteration with (qs, H_filled, k) where qs is the current
                basis list, H_filled is H[:k+1, :k], and k is the iteration index (1-based).
                Used for checkpointing intermediate Arnoldi state.
        initial_qs (list, optional): If provided with initial_H_filled, resume from
                this state instead of starting from b. qs has length k+1 after k
                completed iterations.
        initial_H_filled (tensor, optional): Hessenberg (k+1) x k for resuming.
                Must be provided with initial_qs.
    ...
    dtype = next(iter(b)).dtype

    # Resume from saved state or start from b
    if (
        initial_qs is not None
        and initial_H_filled is not None
        and len(initial_qs) >= 2
    ):
        qs = [(_parameter_to(q, device=projection_device)) for q in initial_qs]
        n_filled = len(qs) - 1  # number of completed iterations
        H = torch.zeros(n + 1, n, dtype=dtype).to(device=projection_device)
        H_filled = initial_H_filled.to(
            device=projection_device, dtype=dtype
        )
        if H_filled.shape[0] != n_filled + 1 or H_filled.shape[1] != n_filled:
            raise ValueError(
                f"initial_H_filled shape {H_filled.shape} inconsistent with "
                f"len(initial_qs)={len(initial_qs)} (expected ({n_filled + 1}, {n_filled}))"
            )
        H[: n_filled + 1, :n_filled] = H_filled
        start_k = n_filled + 1  # next iteration to run (1-based)
        if start_k > n:
            return qs[:n], H[:n, : n - 1]
        iterates = range(start_k, n + 1)
        logging.info(f"Resuming Arnoldi from iteration {start_k}")
    else:
        H = torch.zeros(n + 1, n, dtype=dtype).to(device=projection_device)
        qs = [
            _parameter_to(
    ...
    for k in iterates:
        ...
        if iteration_callback is not None:
            # H_filled: (k+1) x k, same format expected by _parameter_distill
            H_filled = H[: k + 1, :k].clone()
            iteration_callback(qs, H_filled, k)
...
class ArnoldiEmbedder(EmbedderBase):
    ...
    def fit(
        ...
        fit_checkpoint_path: Optional[str] = None,
        fit_resume_state_path: Optional[str] = None,
        initial_arnoldi_state: Optional[
            Tuple[List[Tuple[Tensor, ...]], Tensor]
        ] = None,
    ):
        r"""
        ...
        Args:
            ...
            fit_checkpoint_path (str, optional): If provided, after each Arnoldi
                    iteration a partial fit is saved to this path (overwritten each time).
                    Allows recovery if the run is interrupted during fit.
            fit_resume_state_path (str, optional): If provided, after each Arnoldi
                    iteration the raw (qs, H_filled) state is saved here so the next
                    run can resume from it. Should be cleared by the caller when fit
                    completes successfully.
            initial_arnoldi_state (tuple, optional): (qs, H_filled) from a previous
                    run to resume from. When provided, Arnoldi iteration continues
                    from the saved state instead of from scratch.
        """
        ...
        self.fit_results = self._retrieve_projections_arnoldi_embedder(
            ...
            self.show_progress,
            fit_checkpoint_path=fit_checkpoint_path,
            fit_resume_state_path=fit_resume_state_path,
    ...
    def _retrieve_projections_arnoldi_embedder(
        ...
        with torch.no_grad():
            ...
            # Optional: save partial fit and raw state after each Arnoldi iteration
            iteration_cb = None
            if fit_checkpoint_path is not None or fit_resume_state_path is not None:

                def _checkpoint_callback(
                    qs_cb: List[Tuple[Tensor, ...]],
                    H_filled: Tensor,
                    step_k: int,
                ) -> None:
                    if fit_resume_state_path is not None:
                        try:
                            with open(fit_resume_state_path, "wb") as f:
                                pickle.dump({"qs": qs_cb, "H_filled": H_filled}, f)
                            logging.info(
                                f"Saved Arnoldi resume state at iteration {step_k} "
                                f"to {fit_resume_state_path}"
                            )
                        except Exception as e:
                            logging.warning(
                                f"Failed to save Arnoldi resume state at step {step_k}: {e}"
                            )
                    if fit_checkpoint_path is not None:
                        # _parameter_distill needs at least 2 iterations (H 2x1 -> 1x1)
                        if step_k < 2:
                            return
                        try:
                            k_proj = min(self.projection_dim, step_k)
                            ...
                            ls = (1.0 / ls) ** 0.5
                            R = [
                                _parameter_multiply(v, l)
                                for (v, l) in zip(vs, ls)
                            ]
                            partial_results = ArnoldiEmbedderFitResults(R)
                            with open(fit_checkpoint_path, "wb") as f:
                                pickle.dump(partial_results, f)
                            logging.info(
                                f"Saved Arnoldi fit checkpoint at iteration {step_k} "
                                f"(projection_dim={len(R)}) to {fit_checkpoint_path}"
                            )
                        except Exception as e:
                            logging.warning(
                                f"Failed to save Arnoldi checkpoint at step {step_k}: {e}"
                            )

                iteration_cb = _checkpoint_callback
            ...
            init_qs = None
            init_H = None
            if initial_arnoldi_state is not None:
                init_qs, init_H = initial_arnoldi_state
