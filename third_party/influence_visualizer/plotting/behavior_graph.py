...
from influence_visualizer.plotting.clusters import CLUSTER_COLORS, _get_cluster_color
...
def create_transition_matrix_heatmap(
    ...
    cluster_ids = sorted(graph.cluster_nodes.keys())
...
def create_node_value_bar_chart(
    ...
    cluster_ids = sorted(
        graph.cluster_nodes.keys(), key=lambda nid: values.get(nid, 0), reverse=True,
    )
    terminal_ids = sorted(graph.terminal_node_ids)
...
def create_advantage_matrix_heatmap(
    ...
    gamma: float = 0.99,
    height: int = 500,
    title: str = "Advantage Matrix  A(s, s') = γ·V(s') − V(s)",
) -> go.Figure:
    """Heatmap of the advantage A(s, s') = Q(s,s') - V(s) for each transition.
...
def create_q_value_distribution_plot(
    ...
    node_values: Dict[int, float],
    height: int = 400,
    title: str = "Q-value Distribution per Cluster",
) -> go.Figure:
    """Box plot showing the spread of per-slice Q-values within each cluster.

    Args:
        graph: BehaviorGraph for node names.
        cluster_labels: Per-slice cluster assignments.
        q_values: Per-slice Q-values from ``compute_slice_values()``.
...
def create_episode_value_timeline(
    ...
    max_episodes: int = 30,
    height: int = 500,
    title: str = "Q-value Timeline per Episode",
) -> go.Figure:
    """Line plot of per-slice Q-values along the timestep axis per episode.

    Episodes are sorted by their mean Q-value (successful episodes tend to
    be at the top).

    Args:
        cluster_labels: Per-slice cluster assignments.
        metadata: Per-slice metadata dicts.
        q_values: Per-slice Q-values.
    ...
    # Sort episodes by mean Q descending
    ep_means = {
    ...
    for ep_idx in sorted_eps:
        ...
        fig.add_trace(
            go.Scatter(
                ...
                hovertemplate=f"Ep {ep_idx}<br>t=%{{x}}<br>Q=%{{y:+.3f}}<extra></extra>",
            )
        )
...
def create_episode_q_advantage_timeline(
    ...
    """Line plot of Q(s,s') and Advantage over timesteps for a single episode.

    Filters to slices belonging to the given episode and excludes noise (cluster -1).
    Two traces: Q-value and Advantage.
    ...
    Returns:
        Plotly Figure with two lines (Q and Advantage).
    """
    ep_key = "rollout_idx" if level == "rollout" else "demo_idx"
    ...
    if not points:
        fig = go.Figure()
        fig.update_layout(
            title=title or f"Episode {ep_idx} — Q & Advantage",
            height=height,
        )
        return fig
    ...
    fig.add_trace(
        go.Scatter(
            ...
            name="Q(s, s')",
            hovertemplate="t=%{x}<br>Q=%{y:+.3f}<extra></extra>",
        )
    )
    fig.add_trace(
        go.Scatter(
...
def create_value_heatmap(
    ...
    level: str = "rollout",
    height: int = 600,
    title: str = "Q-value Heatmap (Episodes × Timesteps)",
) -> go.Figure:
    """Heatmap with episodes on y-axis, timesteps on x-axis, colored by Q-value.

    Episodes are sorted by outcome (success first) then by mean Q-value.
    ...
    # Sort: success first, then by mean Q desc
    def _sort_key(ep):
    ...
    fig = go.Figure(
        data=go.Heatmap(
            ...
            zmin=-abs_max,
            zmax=abs_max,
            colorbar_title="Q(s,s')",
            hovertemplate="Episode: %{y}<br>Timestep: %{x}<br>Q = %{z:+.3f}<extra></extra>",
        )
    )
    fig.update_layout(
        title=title,
...
def create_interactive_behavior_graph(
    ...
    layout_algorithm: str = "hierarchicalRepulsion",
) -> str:
    """Create an interactive, draggable graph using Pyvis (vis.js).

    Nodes can be dragged and repositioned. Physics simulation can be
    toggled. Edges show direction and transition probability on hover.
    ...
    Args:
        ...
        layout_algorithm: vis.js solver — "hierarchicalRepulsion",
            "barnesHut", "forceAtlas2Based", "repulsion", or
            "layeredStatic" (mirrors the Plotly graph view layout with
            no physics — drag nodes freely).
...
def create_value_colored_interactive_graph(
    ...
    """Pyvis interactive graph with nodes colored by V(s) and edges by Q(s,s').

    Uses the layered static layout (same as the Plotly Graph View) with
    physics disabled so nodes can be freely dragged.

    Args:
        graph: BehaviorGraph with nodes and transition probabilities.
        values: Dict mapping node_id to V(s).
        gamma: Discount factor for Q-value computation on edges.
        min_probability: Hide edges below this threshold.
    ...
    # Collect Q-values for all visible edges to set the color scale
