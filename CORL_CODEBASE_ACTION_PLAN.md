...
- **RoboCasa low-dim smoke:** skips when no `kitchen_lowdim_merged.hdf5` (or similar) under `data/source/robocasa` — user layout today is mostly LeRobot under `v1.0/`, not robomimic-layout kitchen HDF5.
- **RoboCasa hybrid smoke:** discovers `**/meta/modality.json` under `data/source/robocasa` but skips without a robomimic-layout **eval** `.hdf5`; first discovered LeRobot atomic export may not match `robocasa_lerobot_atomic` `shape_meta` without task-specific discovery or env override.
- **Default MimicGen training file:** Cupid YAML still documents `square_merged.hdf5`; “original” MimicGen pipeline uses `source/square.hdf5` + generated `core_datasets/.../demo.hdf5` — document when to use merged vs source for diffusion (core `demo.hdf5` is **not** interchangeable with low-dim config obs layout).
