"""Lightweight path helpers for seed-specific eval/train directories.

These were originally exported from ``influence_visualizer.data_loader``.
Kept in a separate lightweight module here (no torch / hydra / diffusion_policy
imports) so callers can use the seed-path utilities without paying the cost of
the full ``policy_doctor.influence.loader`` import.
"""
...
def get_train_dir_for_seed(train_dir: str, seed: str, reference_seed: str) -> str:
    ...
    Same logic as :func:`get_eval_dir_for_seed` but for ``train_dir``
    (e.g. ``.../square_mh_0`` → ``.../square_mh_1``).
    """
    if seed == reference_seed:
        return train_dir
