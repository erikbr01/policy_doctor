...
Results are stored under ``<configs_root>/<task_config>/clustering/<name>/`` with
``manifest.yaml``, ``cluster_labels.npy``, and ``metadata.json`` — and optionally
``embeddings_reduced.npy``. Ported from ``influence_visualizer.clustering_results``;
the on-disk layout is preserved so historical runs continue to load.
"""
...
def get_clustering_dir(task_config: str) -> Path:
    ...
    Resolves to ``<iv_task_configs_base>/<task_config>/clustering`` so the legacy
    on-disk location of saved clustering runs is preserved during the refactor.
    """
    from policy_doctor.paths import iv_task_configs_base

    return iv_task_configs_base() / task_config / "clustering"
...
def load_clustering_result_from_path(
    ...
    The directory must contain ``manifest.yaml``, ``cluster_labels.npy``, and
    ``metadata.json``.
...
def load_clustering_result(
    ...
    """Load a clustering result from disk by ``(task_config, name)``.
    ...
    *,
    ...
    Args:
        ...
        output_dir: Explicit output directory. When provided, results are written to
            ``output_dir / slug`` instead of the task-configs tree. Takes precedence
            over task_config.
