...
Ported verbatim from ``third_party/influence_visualizer/data_loader.py`` so the
``policy_doctor`` package no longer depends on the legacy ``influence_visualizer``
distribution. Semantics are unchanged; only iv-internal imports were retargeted to
``policy_doctor.influence.*`` and the dead ``InfluenceDataLoader`` / ``SampleData`` /
``TrajectoryData`` / ``create_mock_influence_data`` surfaces were dropped.
...
from diffusion_policy.dataset.episode_dataset import EpisodeDataset
...
def load_influence_data(
    ...
    if image_dataset_path is not None:
        if lazy_load_images:
            ...
            from policy_doctor.influence.lazy_hdf5 import (
