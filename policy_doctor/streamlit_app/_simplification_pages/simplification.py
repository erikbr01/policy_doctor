"""Graph Simplification — page body.

Loaded by ``graph_simplification_app.py`` via ``st.navigation``. The hub
script handles sys.path bootstrap and ``st.set_page_config``; this file is
the actual simplification page.
"""
...
# parents[3] because this file lives at
# policy_doctor/streamlit_app/_simplification_pages/simplification.py
...
# New canonical location for the W×S×K×rep×ordering sweep results.
...
def _task_from_path(p: Path) -> str:
    parts = p.parts
    for i, part in enumerate(parts):
        if part == "demo_sweep" and i + 1 < len(parts):
            return parts[i + 1]
...
def _ordering_from_manifest(manifest: Dict, p: Path) -> str:
    """Return 'umap_first' or 'aggregate_first'. Prefer the manifest's
    pipeline_steps; fall back on the parent-dir name in demo_sweep."""
    steps = manifest.get("pipeline_steps") or []
...
available: List[Path] = []
for root in [_DEMO_SWEEP, _IV_CONFIGS, _IV_CONFIGS_MAIN, _MAIN_REPO / "e1_experiments"]:
    if root.exists():
...
def _index_entry(p: Path) -> Dict:
    ...
    return {
        "path": p,
        "task": _task_from_path(p),
        "ordering": _ordering_from_manifest(m, p),
        "rep": rep_full,
...
def _filter_index(task=None, ordering=None, rep=None, k=None, w=None, s=None, agg=None) -> List[Dict]:
    out = _index
    if task is not None: out = [e for e in out if e["task"] == task]
...
filtered = _filter_index(task=task_pick, ordering=ordering_pick, rep=rep_pick, k=k_pick, w=w_pick, s=s_pick, agg=agg_pick)
