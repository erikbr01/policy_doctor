...
@st.fragment
def _sil_gamma_panel(K_sil: List[int], sil_vals: List[float]) -> None:
    """Silhouette γ-selection interactive panel.

    Defined at module level so @st.fragment has a stable qualified name —
    otherwise Streamlit can't track the fragment identity between reruns and
    the slider doesn't update the plots.
    """
    ...
    K_fine = [select_K_by_silhouette_gamma(K_sil, sil_vals, gamma=float(g)) for g in g_fine]
    fig_ss = go.Figure()
    fig_ss.add_trace(go.Scatter(
        x=list(g_fine), y=[k if k is not None else float("nan") for k in K_fine],
    ...
    ms4.metric("K* (selected)", str(K_star_sil) if K_star_sil is not None else "none",
        help=("None = monotone decreasing, no K signal." if K_star_sil is None else
              f"Largest K < {peak_K} with silhouette ≥ {target_sil:.3f}."))
    if K_star_sil is None:
        st.warning(
            "⚠ Silhouette peak is at the smallest K — monotone decreasing. "
    ...
    with st.expander(
        ...
        if len(sil_family) < 3:
            st.info(
                ...
                "run `python scripts/run_simplification_model_selection.py "
                "--phase patch_silhouette` to compute them."
            )
        else:
            K_sil = [r["K"] for r in sil_family]
            sil_vals = [r["silhouette"] for r in sil_family]
            _sil_gamma_panel(K_sil, sil_vals)
